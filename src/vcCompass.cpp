...
udResult vcCompass_Create(vcCompass **ppCompass)
{
  ...
  UD_ERROR_CHECK(vcMesh_Create(&pCompass->pMesh, vcNormalVertexLayout, UDARRAYSIZE(vcNormalVertexLayout), compassVerts, UDARRAYSIZE(compassVerts), compassFaces, UDARRAYSIZE(compassFaces), vcMF_IndexShort));
  UD_ERROR_IF(!vcShader_CreateFromText(&pCompass->pShader, g_PositionNormalVertexShader, g_PositionNormalFragmentShader, vcNormalVertexLayout, UDARRAYSIZE(vcNormalVertexLayout)), udR_InvalidConfiguration);
...
udResult vcCompass_Render(vcCompass *pCompass, udDouble4x4 worldViewProj)
{
  ...
  pCompass->shaderBuffer.u_colour = udFloat4::create(1, 1, 0, 1);
  pCompass->shaderBuffer.u_projection = udFloat4x4::create(worldViewProj);
  ...
  UD_ERROR_IF(!vcMesh_RenderTriangles(pCompass->pMesh, UDARRAYSIZE(compassFaces) / 3), udR_InternalError);
