...
vertex main0_out main0(main0_in in [[stage_in]], constant type_u_cameraPlaneParams& u_cameraPlaneParams [[buffer(0)]], constant type_u_EveryObject& u_EveryObject [[buffer(1)]], texture2d<float> demTexture [[texture(0)]], sampler demSampler [[sampler(0)]])
{
    ...
    float4 _158 = u_EveryObject.u_projection * (float4(mix(_113.xyz, (u_EveryObject.u_view * float4(_101 * _113.w, 1.0)).xyz, float3(u_EveryObject.u_objectInfo.z)), 1.0) + ((u_EveryObject.u_view * float4(_100.xyz, 0.0)) * (demTexture.sample(demSampler, _126, level(0.0)).x + (in.in_var_POSITION.z * u_EveryObject.u_objectInfo.y))));
    float _184;
    switch (0u)
    {
        default:
        {
            if (u_EveryObject.u_projection[1u][3u] != 0.0)
            {
                float _176 = _158.w;
                _184 = ((log2(fast::max(9.9999999747524270787835121154785e-07, 1.0 + _176)) * ((u_cameraPlaneParams.u_clipZFar - u_cameraPlaneParams.u_clipZNear) / log2(u_cameraPlaneParams.s_CameraFarPlane + 1.0))) + u_cameraPlaneParams.u_clipZNear) * _176;
                break;
            }
            _184 = _158.z;
            break;
    ...
    out.out_var_TEXCOORD1 = float2(_184, _158.w);
    out.out_var_TEXCOORD2 = _198;
