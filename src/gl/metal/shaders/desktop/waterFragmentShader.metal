...
fragment main0_out main0(main0_in in [[stage_in]], constant type_u_cameraPlaneParams& u_cameraPlaneParams [[buffer(0)]], constant type_u_EveryFrameFrag& u_EveryFrameFrag [[buffer(1)]], texture2d<float> u_normalMap [[texture(0)]], texture2d<float> u_skybox [[texture(1)]], sampler sampler0 [[sampler(0)]], sampler sampler1 [[sampler(1)]])
{
    main0_out out = {};
    float3 _85 = normalize(((u_normalMap.sample(sampler0, in.in_var_TEXCOORD0).xyz * float3(2.0)) - float3(1.0)) + ((u_normalMap.sample(sampler0, in.in_var_TEXCOORD1).xyz * float3(2.0)) - float3(1.0)));
