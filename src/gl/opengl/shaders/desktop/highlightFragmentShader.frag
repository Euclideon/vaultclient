...
void main()
{
    ...
    switch (0u)
    {
        default:
        {
            ...
            if (_48 == 0.0)
            {
                _101 = vec4(in_var_COLOR0.xyz, min(1.0, (_47.y * in_var_COLOR1.z) * in_var_COLOR0.w));
                break;
            }
            float _65 = 0.1500000059604644775390625 * in_var_COLOR0.w;
            _101 = vec4(in_var_COLOR0.xyz, max(in_var_COLOR1.w, ((((1.0 - _47.y) + (_65 * step(texture(SPIRV_Cross_CombinedcolourTexturecolourSampler, in_var_TEXCOORD1).x - _48, -9.9999997473787516355514526367188e-06))) + (_65 * step(texture(SPIRV_Cross_CombinedcolourTexturecolourSampler, in_var_TEXCOORD2).x - _48, -9.9999997473787516355514526367188e-06))) + (_65 * step(texture(SPIRV_Cross_CombinedcolourTexturecolourSampler, in_var_TEXCOORD3).x - _48, -9.9999997473787516355514526367188e-06))) + (_65 * step(texture(SPIRV_Cross_CombinedcolourTexturecolourSampler, in_var_TEXCOORD4).x - _48, -9.9999997473787516355514526367188e-06))) * in_var_COLOR0.w);
            break;
        }
    }
