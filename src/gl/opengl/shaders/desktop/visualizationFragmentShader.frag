...
void main()
{
    ...
    if ((u_fragParams.u_outlineParams.x > 0.0) && (u_fragParams.u_outlineColour.w > 0.0))
    {
        ...
        vec4 _239 = texture(SPIRV_Cross_CombinedsceneDepthTexturesceneDepthSampler, in_var_TEXCOORD4);
        vec4 _241 = texture(SPIRV_Cross_CombinedsceneDepthTexturesceneDepthSampler, in_var_TEXCOORD5);
        float _242 = _235.x;
        float _243 = _237.x;
        float _244 = _239.x;
        float _245 = _241.x;
        float _248 = 0.0005000000237487256526947021484375 + ((u_fragParams.u_outlineParams.y * _92) * 0.001000000047497451305389404296875);
        vec4 _281 = mix(vec4(_219, _92), vec4(mix(_219.xyz, u_fragParams.u_outlineColour.xyz, vec3(u_fragParams.u_outlineColour.w)), min(min(min(_242, _243), _244), _245)), vec4(1.0 - (((step(abs(_242 - _92), _248) * step(abs(_243 - _92), _248)) * step(abs(_244 - _92), _248)) * step(abs(_245 - _92), _248))));
        _284 = _281.w;
