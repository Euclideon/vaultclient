...
bool vcMesh_CreateSimple(vcMesh **ppMesh, const vcSimpleVertex *pVerts, int totalVerts, const int *pIndices, int totalIndices)
{
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(vcSimpleVertex) * totalVerts, pVerts, GL_DYNAMIC_DRAW);
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vcSimpleVertex), (GLvoid*)(0 * sizeof(float)));
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(vcSimpleVertex), (GLvoid*)(3 * sizeof(float)));
