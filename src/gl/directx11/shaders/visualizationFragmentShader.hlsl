...
{  
  float4 sampleDepth0 = sceneDepthTexture.Sample(sceneDepthSampler, input.edgeSampleUV0).x;
  float4 sampleDepth1 = sceneDepthTexture.Sample(sceneDepthSampler, input.edgeSampleUV1).x;
  float4 sampleDepth2 = sceneDepthTexture.Sample(sceneDepthSampler, input.edgeSampleUV2).x;
  float4 sampleDepth3 = sceneDepthTexture.Sample(sceneDepthSampler, input.edgeSampleUV3).x;
  
  float minEdgeThreshold = 0.0005;
  float scalarEdgeThreshold = 0.001; // this is to keep exisiting `edgeOutlineThreshold` values roughly looking the same
