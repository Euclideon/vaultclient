...
bool vcSettings_Load(vcSettings *pSettings, bool forceReset /*= false*/, vcSettingCategory group /*= vcSC_All*/)
{
  ...
  else if (group == vcSC_Docks)
  {
    if (data.Get("dock").IsArray())
    {
	  ...
	  pSettings->window.windowsOpen[vcDocks_History] = data.Get("frames.history").AsBool(true);
	  ...
	  pSettings->window.windowsOpen[vcDocks_HistoryExplorer] = data.Get("frames.historyExplorer").AsBool(true);
