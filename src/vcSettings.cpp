...
bool vcSettings_Load(vcSettings *pSettings, bool forceReset /*= false*/, vcSettingCategory group /*= vcSC_All*/)
{
  ...
  if (group == vcSC_Bindings || group == vcSC_All)
  {
    ...
    vcHotkey::Set(vcB_AddSimpleCrossSection, data.Get("keys.%s", vcHotkey::GetBindName(vcB_AddSimpleCrossSection)).AsInt(SDL_SCANCODE_O));
  }
  ...
  if (group == vcSC_All)
  {
    ...
    for (int v = 0; v < vcMaxViewportCount; v++)
    {
      ...
      pSettings->camera.fieldOfView[v] = data.Get("camera.fieldOfView[%d]", v).AsFloat(data.Get("camera.fielOfView").AsFloat(vcLens30mm));
