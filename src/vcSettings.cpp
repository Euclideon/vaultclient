...
bool vcSettings_Load(vcSettings *pSettings, bool forceReset /*= false*/, vcSettingCategory group /*= vcSC_All*/)
{
  ...
  else if (group == vcSC_Docks)
  {
    if (data.Get("dock").IsArray())
    {
	  ...
	  pSettings->window.windowsOpen[vcDocks_History] = data.Get("frames.history").AsBool(false);
	  ...
	  pSettings->window.windowsOpen[vcDocks_HistoryExplorer] = data.Get("frames.historyExplorer").AsBool(false);
...
bool vcSettings_Save(vcSettings *pSettings)
{
  ...
  data.Set("frames.history = %s", pSettings->window.windowsOpen[vcDocks_History] ? "true" : "false");
  ...
  data.Set("frames.historyExplorer = %s", pSettings->window.windowsOpen[vcDocks_HistoryExplorer] ? "true" : "false");
