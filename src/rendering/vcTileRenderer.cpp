  ...
  while (pCache->keepLoading)
  {
    ...
    while (pCache->tileLoadList.length > 0 && pCache->keepLoading)
    {
	  ...
	  case vcMTSF_Slippy:
		  ...
		  break;
	  case vcMTSF_Google:
		  udSprintf(localFileName, "%s?lyrs=s&hl=zh-CN&gl=CN&x=%d&y=%d&z=%d", pRenderer->pSettings->cacheAssetPath, pBestNode->slippyPosition.x, pBestNode->slippyPosition.y, pBestNode->slippyPosition.z);
		  udSprintf(serverAddress, "%s?lyrs=s&hl=zh-CN&gl=CN&x=%d&y=%d&z=%d", pRenderer->pSettings->maptiles.tileServerAddress, pBestNode->slippyPosition.x, pBestNode->slippyPosition.y, pBestNode->slippyPosition.z);
		  break;
	  default:
		  break;
	  }
