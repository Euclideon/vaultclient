...
void vcModals_SetTileImage(void *pProgramStatePtr)
{
  ...
  case vcMTSF_Slippy:
      ...
      break;
  case vcMTSF_Google:
      udSprintf(buf, "%s?lyrs=%s&x=0&y=0&z=0", pProgramState->settings.maptiles.tileServerAddress, vcSettings_GetMapTileTypeName(&pProgramState->settings));
      break;
  default:
      udSprintf(buf, "%s", pProgramState->settings.maptiles.tileServerAddress);
      break;
  }
...
void vcModals_DrawTileServer(vcState *pProgramState)
{
  ...
  if (ImGui::BeginPopupModal(vcString::Get("settingsMapsTileServerTitle")))
  {
    ...
    if (s_currentItem == -1)
    {
        ...
        {
            s_currentItem = pProgramState->settings.maptiles.mapTileType;
        }
    }
