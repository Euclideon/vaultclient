...
{
  if (!m_selected || m_line.selectedPoint != -1 || m_pProgramState->pActiveViewport->gizmo.operation != vcGO_Rotate)
...
{
  if (m_line.selectedPoint == -1)
    return udDouble4x4::translation(m_pProgramState->pActiveViewport->gizmo.operation == vcGO_Rotate ? m_gizmoPivot : m_pLabelInfo->worldPosition);
  else
